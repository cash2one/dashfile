...
flag = sys.argv[3]
if flag=='fail':
    infile = "/home/work/data_bu/%s.instance.dump" %  cluster
else:
    ...
    for k1,v1 in ins_map.iteritems():
        ...
        if 'status' in ins_sons:
            ...
            if 'runtime' in ins_status:
                ...
                if 'cur_freeze' in ins_runtime:
                    cur_freeze = ins_runtime['cur_freeze']
        print >>flist, "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
                ins_id, module, app_id, hostname, run_state, work_path, cur_freeze, begin_port)
flist.close()
